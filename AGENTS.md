...
4. **Shared browser context**: During warmup, both Outlook and Harvest share a single Playwright browser context (one Chrome window, two tabs). The `captureToken()` functions accept an optional `existingContext` parameter. Jira doesn't use browser auth.
